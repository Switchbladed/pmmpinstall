...
LINUX_BUILD_URL="https://jenkins.pmmp.io/job/PHP-7.2-Linux-x86_64/lastSuccessfulBuild/artifact/PHP_Linux-x86_64.tar.gz"
...
if [ "$update" == "on" ]; then
	...
	do
		...
		if [ "$(uname -s)" == "Darwin" ]; then
			...
			download_file "$LINUX_BUILD_URL" | tar -zx > /dev/null 2>&1
